...
import com.example.tarea_api.models.Moto;
import com.example.tarea_api.models.MotoModificada; // Añade este import
    ...
    private void fetchVehiculosFromApi(RecyclerView recyclerView) {
        ...
        JsonObjectRequest request = new JsonObjectRequest(
                ...
                response -> {
                    ...
                    // Filtrar solo motos (no modificadas)
                    List<Vehiculo> motos = new ArrayList<>();
                    for (Vehiculo v : apiResponse.getVehiculos()) {
                        if (v instanceof Moto && !(v instanceof MotoModificada)) {
                            motos.add(v);
                        }
                    }

                    setupRecyclerView(recyclerView, motos);
                },
                error -> Toast.makeText(this, "Error al cargar motos", Toast.LENGTH_SHORT).show()
        );
